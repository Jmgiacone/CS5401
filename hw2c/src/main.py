...
def main():
    ...
    for run in range(num_runs):
        ...
        run_best_score, run_best_world, run_best_pac_man, run_best_ghost = \
            single_run(config, world_file_path + "/" + "run" + str(run + 1), log_file)
        log_file.write("\n")
        ...
        if best_pac_man is None:
            best_pac_man = run_best_pac_man
        elif run_best_pac_man.fitness > best_pac_man.fitness:
            best_pac_man = run_best_pac_man
            
        if best_ghost is None:
            best_ghost = run_best_ghost
        elif run_best_ghost.fitness > best_ghost.fitness:
            best_ghost = run_best_ghost

    solution_file = open(str(config["Logging Config"]["Solution File"]), "w+")
    solution_file.write("Best Pac-man GPTree:\n")
    solution_file.write(GPTree.print_level_order(best_pac_man))
    solution_file.write("\nBest Ghost GPTRee:\n")
    solution_file.write(GPTree.print_level_order(best_ghost))
...
def play_game(rows, columns, wall_density, pill_density, fruit_chance, fruit_score, time_multiplier, world_file,
              num_pac_men, num_ghosts, pac_men_gp_trees, ghost_gp_trees):
    ...
    for i in range(num_pac_men):
        ...
        pac_men.append(Individual(start, i + 1, pac_men_gp_trees[i], True))

    start = closest_symbol(board, (rows - 1, columns - 1), ".")

    for i in range(num_ghosts):
        # Ghosts don't get a GPTree
        ghosts.append(Individual(start, i+1, ghost_gp_trees[i], False))
...
def evaluate_population_fitness(num_evals, pac_man_population, num_pac_men, rows, columns, wall_density, pill_density,
                                fruit_chance, fruit_score, time_multiplier, file_path, ghost_population, num_ghosts,
                                parsimony_coefficient, same_controller_for_each_pac_man, same_controller_for_each_ghost):
    """BONUS 1 and 2: This function was also written with the bonuses in mind. Based on the number of simultaneous pac-men, this
    function will partition the controllers up and pass them to the game-playing function"""
    pac_man_population_size = len(pac_man_population)
    pac_man_leftovers = pac_man_population_size % num_pac_men
    ghost_population_size = len(ghost_population)
    ghost_leftovers = ghost_population_size % num_ghosts
    pac_man_groups = []
    ghost_groups = []
    current_max_score = 0
    best_game = ""
    
    if same_controller_for_each_pac_man:
        for pac_man in pac_man_population:
            pac_man_groups.append([pac_man] * num_pac_men)
    else:
        for i in range(0, pac_man_population_size - pac_man_leftovers, num_pac_men):
            group = []
            for j in range(num_pac_men):
                group.append(pac_man_population[i + j])
            pac_man_groups.append(group)
    
        if pac_man_leftovers != 0:
            group = pac_man_population[-pac_man_leftovers:]
            
            for i in range(num_pac_men - pac_man_leftovers):
                group.append(pac_man_population[i])
            
            pac_man_groups.append(group)

    if same_controller_for_each_ghost:
        for ghost in ghost_population:
            ghost_groups.append([ghost] * num_ghosts)
    else:
        for i in range(0, ghost_population_size - ghost_leftovers, num_ghosts):
            group = []
            for j in range(num_ghosts):
                group.append(ghost_population[i + j])
            ghost_groups.append(group)

        if ghost_leftovers != 0:
            group = ghost_population[-ghost_leftovers:]

            for i in range(num_ghosts - ghost_leftovers):
                group.append(ghost_population[i])

            ghost_groups.append(group)
     
    num_pac_man_groups = len(pac_man_groups)
    num_ghost_groups = len(ghost_groups)   
    
    loop_limit = max(num_pac_man_groups, num_ghost_groups)
    
    pac_man_index = ghost_index = 0
    for i in range(loop_limit):
        # Play a game with the groups
        num_evals += 1
        ...
        score, game = play_game(rows, columns, wall_density, pill_density, fruit_chance, fruit_score,
                                ...
                                pac_man_groups[pac_man_index], ghost_groups[ghost_index])
        
        # Apply parsimony pressure
        for pac_man in pac_man_groups[pac_man_index]:
            if pac_man.fitness is not None:
                # Yank out that denominator so we can add on the new fitness
                pac_man.fitness *= pac_man.num_games
                
                # Increment "denominator"
                pac_man.num_games += 1
                
                # Add in new score
                pac_man.fitness += score - parsimony_coefficient * pac_man.num_nodes
        ...
        for ghost in ghost_groups[ghost_index]:
            if ghost.fitness is not None:
                # Yank out that denominator so we can add on the new fitness
                ghost.fitness *= ghost.num_games

                # Increment "denominator"
                ghost.num_games += 1

                # Add in new score
                ghost.fitness += 2000 - (score + parsimony_coefficient * ghost.num_nodes)
        ...
        # Make sure the counters don't overflow
        pac_man_index %= num_pac_man_groups
        ghost_index %= num_ghost_groups

    # Remove extra var from pac-men and ghosts
    for pac_man in pac_man_population:
        pac_man.num_games = 1
    ...
    return current_max_score, best_game, num_evals
...
def single_run(config, file_path, log_file):
    ...
    # EA params - Pac-man and ghosts separately
    # TODO: Update config file
    pac_man_population_size = int(config["EA Config"]["Pac-Man Population Size"])
    ghost_population_size = int(config["EA Config"]["Ghost Population Size"])
    pac_man_offspring_per_generation = int(config["EA Config"]["Pac-Man Offspring per Generation"])
    ghost_offspring_per_generation = int(config["EA Config"]["Ghost Offspring per Generation"])
    pac_man_parent_selection_method = ParentSelectionMethod(int(config["EA Config"]["Pac-Man Parent Selection Method"]))
    ghost_parent_selection_method = ParentSelectionMethod(int(config["EA Config"]["Ghost Parent Selection Method"]))
    pac_man_survival_selection_method = \
        SurvivalSelectionMethod(int(config["EA Config"]["Pac-Man Survival Selection Method"]))
    ghost_survival_selection_method = \
        SurvivalSelectionMethod(int(config["EA Config"]["Ghost Survival Selection Method"]))
    ...
    pac_man_same_controller_for_each = config["EA Config"]["Multiple Pac-Men use one controller"] == "true"
    ghost_same_controller_for_each = config["EA Config"]["Multiple Ghosts use one controller"] == "true"
    ...
    pac_man_population = ramped_half_and_half(pac_man_population_size, max_depth)
    ghost_population = ramped_half_and_half(ghost_population_size, max_depth)
    total_pac_men += pac_man_population_size
    total_ghosts += ghost_population_size

    fitness, game, num_evals = evaluate_population_fitness(num_evals, pac_man_population, num_pac_men, rows, columns, 
                                                           wall_density, pill_density, fruit_chance, fruit_score, 
                                                           time_multiplier, file_path, ghost_population, num_ghosts, 
                                                           parsimony_coefficient, pac_man_same_controller_for_each, 
                                                           ghost_same_controller_for_each)
    # Check for best game
    if fitness > current_best_game_score:
        current_best_game_score = fitness
        current_best_world_file = game

    average_pac_man_fitness, best_pac_man = find_average_fitness_and_best_member(pac_man_population)
    best_ghost = find_average_fitness_and_best_member(ghost_population)[1]

    generation = 1
    log_file.write("{}\t{}\t{}\n".format(num_evals, average_pac_man_fitness, best_pac_man.fitness))
    print("{}\t{}\t{}".format(num_evals, average_pac_man_fitness, best_pac_man.fitness))

    # Only terminate on evaluations
    while num_evals < evals_per_run:
        print("Generation {} ({})".format(generation, num_evals))
        previous_best_pac_man = best_pac_man
        previous_best_ghost = best_ghost

        pac_man_parents = parent_selection(pac_man_population, 2 * pac_man_offspring_per_generation, 
                                           pac_man_parent_selection_method, overselection_i_value, overselection_j_value)
        
        ghost_parents = parent_selection(ghost_population, 2 * ghost_offspring_per_generation, 
                                         ghost_parent_selection_method, overselection_i_value, overselection_j_value)

        # Pair up pac-man parents and create offspring
        pac_man_children = []
        for i in range(0, 2 * pac_man_offspring_per_generation, 2):
            if uniform(0, 1) < mutation_chance:
                # Perform mutation on a clone of either parent
                child = deepcopy(pac_man_parents[i + randrange(2)])
                child.fitness = None
                # print("Mutate individual {}".format(child.id))

                GPTree.subtree_mutation(child, child.max_depth)
                total_pac_men += 1
                child.id = total_pac_men
                pac_man_children.append(child)
            else:
                # Subtree crossover
                # print("Crossover individuals {} and {}".format(pac_man_parents[i].id, pac_man_parents[i + 1].id))
                child = GPTree.sub_tree_crossover(pac_man_parents[i], pac_man_parents[i + 1])
                total_pac_men += 1
                child.id = total_pac_men
                pac_man_children.append(child)
            # GPTree.print_level_order(child)

        # Pair up ghost parents and create offspring
        ghost_children = []
        for i in range(0, 2 * ghost_offspring_per_generation, 2):
            if uniform(0, 1) < mutation_chance:
                # Perform mutation on a clone of either parent
                child = deepcopy(ghost_parents[i + randrange(2)])
                child.fitness = None
                # print("Mutate individual {}".format(child.id))

                GPTree.subtree_mutation(child, child.max_depth)
                total_ghosts += 1
                child.id = total_ghosts
                ghost_children.append(child)
            else:
                # Subtree crossover
                # print("Crossover individuals {} and {}".format(ghost_parents[i].id, ghost_parents[i + 1].id))
                child = GPTree.sub_tree_crossover(ghost_parents[i], ghost_parents[i + 1])
                total_ghosts += 1
                child.id = total_ghosts
                ghost_children.append(child)
                # GPTree.print_level_order(child)

        fitness, game, num_evals = evaluate_population_fitness(num_evals, pac_man_children, num_pac_men, rows, columns,
                                                               wall_density, pill_density, fruit_chance, fruit_score, 
                                                               time_multiplier, file_path, ghost_children, num_ghosts, 
                                                               parsimony_coefficient, pac_man_same_controller_for_each,
                                                               ghost_same_controller_for_each)
        ...
        pac_man_population = survival_selection(pac_man_population, pac_man_children, len(pac_man_population),
                                                tournament_selection_size, pac_man_survival_selection_method)
        ghost_population = survival_selection(ghost_population, ghost_children, len(ghost_population),
                                              tournament_selection_size, ghost_survival_selection_method)
    ...
    return current_best_game_score, current_best_world_file, best_pac_man, best_ghost
